...
public class CertTrustMeHook {
	...
	public void handleLoadPackage(final LoadPackageParam lpparam)
		...
		/* Apache Hooks */
		/*
		 * external/apache-http/src/org/apache/http/impl/client/DefaultHttpClient
		 * .java
		 */
		...
		Log.d(TAG, "Hooking DefaultHTTPClient(HttpParams) for: "
				+ currentPackageName);
		findAndHookConstructor(DefaultHttpClient.class, HttpParams.class,
				new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
							throws Throwable {

						setObjectField(param.thisObject, "defaultParams",
								(HttpParams) param.args[0]);
						setObjectField(param.thisObject, "connManager",
								getSCCM());
					}
				});
		 ...
		 */
		/*
		 * public DefaultHttpClient(ClientConnectionManager conman, HttpParams
		 * params)
		 */
		Log.d(TAG,
		...
		Log.d(TAG,
				"Hooking SSLSocketFactory(String, KeyStore, String, KeyStore) for: "
						+ currentPackageName);
		findAndHookConstructor(SSLSocketFactory.class, String.class,
				KeyStore.class, String.class, KeyStore.class,
				...
				new XC_MethodHook() {
					...
					protected void afterHookedMethod(MethodHookParam param)
						...
						callMethod(
								getObjectField(param.thisObject, "sslcontext"),
		...
		findAndHookMethod("org.apache.http.conn.ssl.SSLSocketFactory",
				lpparam.classLoader, "getSocketFactory",
		...
		Log.d(TAG, "Hooking SSLSocketFactory(Socket) for: "
				+ currentPackageName);
		findAndHookMethod("org.apache.http.conn.ssl.SSLSocketFactory",
				lpparam.classLoader, "isSecure", Socket.class,
				new XC_MethodReplacement() {
					@Override
					protected Object replaceHookedMethod(MethodHookParam param)
							throws Throwable {
		...
		findAndHookMethod("javax.net.ssl.TrustManagerFactory",
				lpparam.classLoader, "getTrustManagers", new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param)
							throws Throwable {
						...
						if (hasTrustManagerImpl()) {
							...
							TrustManager[] managers = (TrustManager[]) param
									.getResult();
							if (managers.length > 0
									&& cls.isInstance(managers[0]))
								return;
						}
		...
		findAndHookMethod("javax.net.ssl.HttpsURLConnection",
				lpparam.classLoader, "setDefaultHostnameVerifier",
		...
		Log.d(TAG, "Hooking HttpsURLConnection.setSSLSocketFactory for: "
				+ currentPackageName);
		findAndHookMethod("javax.net.ssl.HttpsURLConnection",
				lpparam.classLoader, "setSSLSocketFactory",
		...
		Log.d(TAG, "Hooking HttpsURLConnection.setHostnameVerifier for: "
				+ currentPackageName);
		findAndHookMethod("javax.net.ssl.HttpsURLConnection",
				lpparam.classLoader, "setHostnameVerifier",
				HostnameVerifier.class, new XC_MethodReplacement() {
					@Override
					protected Object replaceHookedMethod(MethodHookParam param)
							throws Throwable {
		...
		findAndHookMethod("android.webkit.WebViewClient", lpparam.classLoader,
				"onReceivedSslError", WebView.class, SslErrorHandler.class,
				SslError.class, new XC_MethodReplacement() {
					@Override
					protected Object replaceHookedMethod(MethodHookParam param)
							throws Throwable {
						((android.webkit.SslErrorHandler) param.args[1])
								.proceed();
						return null;
					}
				});
		...
		findAndHookMethod("android.webkit.WebViewClient", lpparam.classLoader,
				"onReceivedError", WebView.class, int.class, String.class,
		...
		// Multi-dex support:
		// https://github.com/rovo89/XposedBridge/issues/30#issuecomment-68486449
		...
		if (hasTrustManagerImpl()) {
			...
			/*
			 * external/conscrypt/src/platform/java/org/conscrypt/TrustManagerImpl
			 * .java
			 */
			Log.d(TAG,
			...
			/*
			 * public List<X509Certificate> checkServerTrusted(X509Certificate[]
			 * chain, String authType, String host) throws CertificateException
			 */
			findAndHookMethod("com.android.org.conscrypt.TrustManagerImpl",
					lpparam.classLoader, "checkServerTrusted",
			...
			/*
			 * public List<X509Certificate> checkServerTrusted(X509Certificate[]
			 * chain, String authType, SSLSession session) throws
			 * CertificateException
			 */
			findAndHookMethod("com.android.org.conscrypt.TrustManagerImpl",
					lpparam.classLoader, "checkServerTrusted",
					X509Certificate[].class, String.class, SSLSession.class,
					new XC_MethodReplacement() {
						@Override
						protected Object replaceHookedMethod(
								MethodHookParam param) throws Throwable {
	...
	public ClientConnectionManager getSCCM() {
		...
		try {
			...
			ClientConnectionManager ccm = new SingleClientConnManager(null,
					registry);
	...
	void processOkHttp(ClassLoader classLoader) {
		...
		/*
		 * com/squareup/okhttp/CertificatePinner.java available online @
		 * https://
		 * github.com/square/okhttp/blob/master/okhttp/src/main/java/com/
		 * squareup/okhttp/CertificatePinner.java
		 */
		/*
		 * public void check(String hostname, List<Certificate>
		 * peerCertificates) throws SSLPeerUnverifiedException{}
		 */
		...
		try {
			...
			findAndHookMethod("com.squareup.okhttp.CertificatePinner",
					classLoader, "check", String.class, List.class,
					new XC_MethodReplacement() {
						@Override
						protected Object replaceHookedMethod(
			...
			// pass
			Log.d(TAG, "OKHTTP 2.5 not found in " + currentPackageName
					+ "-- not hooking");
		}
		...
		try {
			...
			findAndHookMethod("okhttp3.internal.tls.OkHostnameVerifier",
					classLoader, "verify", String.class,
					javax.net.ssl.SSLSession.class, new XC_MethodReplacement() {
						@Override
						protected Object replaceHookedMethod(
								MethodHookParam methodHookParam)
								throws Throwable {
	...
	void processHttpClientAndroidLib(ClassLoader classLoader) {
		...
		/*
		 * public final void verify(String host, String[] cns, String[]
		 * subjectAlts, boolean strictWithSubDomains) throws SSLException
		 */
		Log.d(TAG,
		...
		try {
			classLoader
					.loadClass("ch.boye.httpclientandroidlib.conn.ssl.AbstractVerifier");
			findAndHookMethod(
					"ch.boye.httpclientandroidlib.conn.ssl.AbstractVerifier",
					classLoader, "verify", String.class, String[].class,
			...
			// pass
			Log.d(TAG, "httpclientandroidlib not found in "
					+ currentPackageName + "-- not hooking");
		}
	}
